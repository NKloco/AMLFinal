"""
This module will provide the functionality required to recreate the results of the deep temporal clustering representation(DTCR) algorithm.
"""
import math
import random
...
def create_fake_time_series(sample, fake_alpha=FAKE_SAMPLE_ALPHA):
    ...
    while len(indices_to_shuffle) > 1:
        ...
        random_index_to_swap = indices_to_shuffle[random.randint(0, len(indices_to_shuffle) - 1)]
...
def main():
    ...
    # Creating a sample with 1000 values to check the fake sampling functionality.
    sample = range(1000)
    ...
    print("For a sample of length {}, there were {} shuffles, which is {} of the series.".format(len(sample), shuffled_count, shuffled_count/len(sample)))
